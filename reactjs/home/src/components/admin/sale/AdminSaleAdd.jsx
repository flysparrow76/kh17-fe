...
export default function AdminSaleAdd(){
    ...
    useEffect(()=>{
        ...
        //파일선택창은 비어있는 value밖에 줄 수 없어서 리엑트에서 모든 상황을 제어할 수 없다(HTML보안 이슈)
        //태그를 직접 제어하는 방향으로 우회 처리한다(ref사용)
    ...
    const sendData = useCallback(async()=>{
        //discount가 false이면 sale에서 saleDiscountPrice를 제거
        //-원본을 절대로 지우면 안됨
        // const copy = {...sale};
        // if(discount === false)
        ...
        //-<form>대산 FormData를 쓰고,<input>대신 append를 이용해서 key=value를 추가
        //-copy를 FormData로 변환한 뒤 전송하면 파일도 이곳에 첨부한다
        ...
        //썸네일 form에 추가(데이터와 파일을 같은레벨로 처리)
        form.append("saleThumbnail",thumbnail);
            ...
            <Row className="mt-5">
                ...
                <Col sm={9}>
                    <Form.Control type="text" name="saleDiscountPrice" value={sale.saleDiscountPrice}
                        onChange={changeNumericValue} placeholder="e.g.,19900000"/>
                </Col>
            </Row>
        )}
        ...
        <Row className="mt-5">
            ...
            <Col sm={9}>
                <Form.Control as="textarea" rows={6} 
                            type="text" name="saleContent" value={sale.saleContent}
                            onChange={changeStringValue} placeholder="e.g.,상품에 대한 설명 작성"/>
            </Col>
        </Row>
        ...
        <Row className="mt-5">
            ...
            <Col sm={9}>
                <div className="d-flex">
                    ...
                    {thumbnail !== null &&(
                        <Button className="ms-2" variant="danger" onClick={clearThumbnail}>
                            <FaXmark/>
                        </Button>
                    )}
